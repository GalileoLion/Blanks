...
fn load_json_with_default(path: &Path, default_value: Value) -> Result<Value, Box<dyn std::error::Error>> {
  if path.exists() {
...
fn merge_missing_keys(value: &mut Value, defaults: &Value) {
  if let (Value::Object(value_map), Value::Object(default_map)) = (value, defaults) {
    for (key, default_value) in default_map {
      value_map.entry(key.clone()).or_insert_with(|| default_value.clone());
    }
  }
}
...
#[tauri::command]
fn startup_log(state: State<AppState>, entry: String) -> Result<(), String> {
  let path = state.user_data_path.join("startup.log");
  let mut file = fs::OpenOptions::new()
    .create(true)
    .append(true)
    .open(path)
    .map_err(map_io)?;
  writeln!(file, "{entry}").map_err(map_io)
}

fn json_object(value: &Value) -> Result<&Map<String, Value>, String> {
  value.as_object().ok_or_else(|| "expected object payload".to_string())
}

fn merge_object(target: &mut Value, patch: &Value) -> Result<(), String> {
  let target = target.as_object_mut().ok_or_else(|| "stored JSON is not an object".to_string())?;
...
#[tauri::command]
fn ipc_send(app: AppHandle, window: Window, state: State<AppState>, channel: String, args: Vec<Value>) -> Result<(), String> {
  match channel.as_str() {
    "mt::ask-for-user-preference" | "mt::ask-for-user-data" => {
      window.emit("mt::user-preference", combined_preferences(&state)?).map_err(|e| e.to_string())?;
    }
    "mt::set-user-preference" => {
      let patch = args.first().ok_or_else(|| "missing preference payload".to_string())?;
      let mut preferences = state.preferences.lock().map_err(|e| e.to_string())?;
      merge_object(&mut preferences, patch)?;
      save_json(&state.preferences_path, &preferences).map_err(|e| e.to_string())?;
      window.emit("mt::user-preference", patch.clone()).map_err(|e| e.to_string())?;
    }
    "mt::set-user-data" => {
      let patch = args.first().ok_or_else(|| "missing data payload".to_string())?;
      let mut data = state.data.lock().map_err(|e| e.to_string())?;
      merge_object(&mut data, patch)?;
    ...
    "mt::get-current-language" => {
      let language = state
        ...
        .unwrap_or("en")
        .to_string();
      window.emit("mt::current-language", language).map_err(|e| e.to_string())?;
    }
    "mt::select-default-directory-to-open" => {
      if let Some(path) = rfd::FileDialog::new().pick_folder() {
        ...
        merge_object(&mut preferences, &patch)?;
        save_json(&state.preferences_path, &preferences).map_err(|e| e.to_string())?;
        window.emit("mt::user-preference", patch).map_err(|e| e.to_string())?;
      }
    }
    "mt::ask-for-modify-image-folder-path" => {
      let selected = args.first().and_then(Value::as_str).map(PathBuf::from).or_else(|| rfd::FileDialog::new().pick_folder());
    ...
    "mt::open-file" | "mt::open-file-by-window-id" | "app-open-file-by-id" => {
      let path = args.iter().rev().find_map(Value::as_str).ok_or_else(|| "missing file path".to_string())?;
      open_markdown_in_window(&window, &state, PathBuf::from(path))?;
    }
    "app-open-directory-by-id" => {
      let path = args.iter().find_map(Value::as_str).ok_or_else(|| "missing directory path".to_string())?;
      open_directory_in_window(&window, PathBuf::from(path))?;
    }
    "mt::response-file-save" | "mt::response-file-save-as" => {
      let _ = handle_save_response(&window, &state, args, channel == "mt::response-file-save-as")?;
    }
    "mt::add-recently-used-document" | "menu-add-recently-used" => {
      let path = args.first().and_then(Value::as_str).ok_or_else(|| "missing recent document path".to_string())?;
    ...
    "mt::editor-ask-file-save" | "mt::editor-ask-file-save-as" | "mt::editor-close-tab" | "mt::tabs-cycle-left" | "mt::tabs-cycle-right"
    | "mt::show-command-palette" | "mt::invalidate-image-cache" => {
      window.emit(channel.as_str(), args).map_err(|e| e.to_string())?;
    }
    "mt::check-for-update" => {
      window
        .emit("mt::UPDATE_NOT_AVAILABLE", "Tauri build does not have an updater endpoint configured.".to_string())
        .map_err(|e| e.to_string())?;
    }
    "mt::NEED_UPDATE" => {}
    "mt::cmd-import-file" => {
    ...
    "mt::window-toggle-always-on-top" => {
      ...
      window.set_always_on_top(next_value).map_err(|e| e.to_string())?;
    }
    "mt::make-screenshot" => {
    ...
    "mt::response-print" => {
      window.emit("mt::tauri-print", Value::Null).map_err(|e| e.to_string())?;
    }
    ...
    "mt::save-and-close-tabs" => {
      ...
      if !saved_ids.is_empty() {
        window.emit("mt::force-close-tabs-by-id", saved_ids).map_err(|e| e.to_string())?;
      }
    }
    ...
    "mt::keybinding-debug-dump-keyboard-info" => {
      ...
      fs::write(&dump_path, serde_json::to_string_pretty(&json!({
        "layout": "browser",
        "keymap": {},
        "platform": env::consts::OS
      })).map_err(|e| e.to_string())?).map_err(map_io)?;
      open::that(dump_path).map_err(|e| e.to_string())?;
    }
    "mt::set-line-ending" => {
      if let Some(line_ending) = args.first().and_then(Value::as_str) {
        window.emit("mt::set-line-ending", line_ending).map_err(|e| e.to_string())?;
      }
    }
    "mt::update-line-ending-menu" | "mt::update-format-menu" | "mt::editor-selection-changed" | "mt::update-sidebar-menu" | "mt::request-keybindings"
    | "mt::handle-renderer-error" => {}
    "mt::view-layout-changed" => {
      let patch = args.get(1).or_else(|| args.first()).ok_or_else(|| "missing layout payload".to_string())?;
      let mut preferences = state.preferences.lock().map_err(|e| e.to_string())?;
      merge_object(&mut preferences, patch)?;
      save_json(&state.preferences_path, &preferences).map_err(|e| e.to_string())?;
    }
...
#[tauri::command]
fn ipc_invoke(window: Window, state: State<AppState>, channel: String, args: Vec<Value>) -> Result<Value, String> {
  match channel.as_str() {
    "mt::fs-trash-item" => {
      let path = args.first().and_then(Value::as_str).ok_or_else(|| "missing path".to_string())?;
      trash::delete(path).map_err(|e| e.to_string())?;
      Ok(Value::Bool(true))
    }
    "mt::get-recently-used-documents" => {
      let data = state.data.lock().map_err(|e| e.to_string())?;
      Ok(data.get("recentDocuments").cloned().unwrap_or_else(|| json!([])))
    }
    ...
    "mt::spellchecker-remove-word" => {
      let word = args.first().and_then(Value::as_str).ok_or_else(|| "missing word".to_string())?;
    ...
    other => {
      let _ = window;
      let _ = state;
      Err(format!("Tauri backend has no ipc_invoke handler for {other}"))
    }
  }
}
...
fn open_directory_in_window(window: &Window, path: PathBuf) -> Result<(), String> {
  window.emit("mt::open-directory", path.to_string_lossy().to_string()).map_err(|e| e.to_string())?;
...
fn scan_project_tree(window: &Window, root: &Path, dir: &Path) -> Result<(), String> {
  let mut entries = fs::read_dir(dir).map_err(map_io)?.collect::<Result<Vec<_>, _>>().map_err(map_io)?;
  ...
  for entry in entries {
    ...
    if metadata.is_dir() {
      if path != root {
        window.emit("mt::update-object-tree", json!({ "type": "addDir", "change": payload })).map_err(|e| e.to_string())?;
      }
      scan_project_tree(window, root, &path)?;
    } else {
      window.emit("mt::update-object-tree", json!({ "type": "add", "change": payload })).map_err(|e| e.to_string())?;
    }
  }
  Ok(())
}
...
fn has_markdown_extension(path: &Path) -> bool {
  ...
  [".markdown", ".mdown", ".mkdn", ".mkd", ".md", ".rmd", ".qmd", ".txt"]
    .iter()
        ...
        if let Some(pair) = entry.as_array() {
          if pair.len() == 2 {
            if let (Some(key), Some(accelerator)) = (pair[0].as_str(), pair[1].as_str()) {
              normalized.insert(key.to_string(), Value::String(accelerator.to_string()));
            }
          }
        }
      }
...
fn remove_custom_dictionary_word(state: &AppState, word: &str) -> Result<(), String> {
  let mut words = load_custom_dictionary(state)?.as_array().cloned().unwrap_or_default();
...
fn handle_export_response(window: &Window, args: Vec<Value>) -> Result<(), String> {
  let payload = args.first().ok_or_else(|| "missing export payload".to_string())?;
  ...
  let content = payload.get("content").and_then(Value::as_str).ok_or_else(|| "missing export content".to_string())?;
  let pathname = payload.get("pathname").and_then(Value::as_str).unwrap_or("");
  let title = payload.get("title").and_then(Value::as_str).unwrap_or("Untitled");
  let extension = if export_type == "styledHtml" { "html" } else { "html" };
  let default_dir = if pathname.is_empty() {
    dirs::document_dir().unwrap_or_else(env::temp_dir)
  } else {
    PathBuf::from(pathname).parent().map(Path::to_path_buf).unwrap_or_else(env::temp_dir)
  };
...
fn emit_notification(window: &Window, title: &str, notification_type: &str, message: &str) -> Result<(), String> {
  window
    .emit(
...
fn handle_image_auto_path(window: &Window, args: Vec<Value>) -> Result<(), String> {
  let payload = args.first().ok_or_else(|| "missing image path payload".to_string())?;
  let pathname = payload.get("pathname").and_then(Value::as_str).unwrap_or("");
  ...
  let full_path = if Path::new(src).is_absolute() {
    ...
    PathBuf::from(pathname).parent().unwrap_or(Path::new("")).join(src)
  };
  let (directory, search_key) = if src.ends_with(std::path::MAIN_SEPARATOR) || src.ends_with('/') || src.ends_with('\\') {
    (full_path, String::new())
  } else {
    (
      full_path.parent().map(Path::to_path_buf).unwrap_or_else(|| PathBuf::from(".")),
      full_path.file_name().map(|value| value.to_string_lossy().to_string()).unwrap_or_default(),
    )
  };
...
fn image_path_candidates(directory: &Path, search_key: &str) -> io::Result<Vec<Value>> {
  ...
  values.sort_by_key(|value| value.get("file").and_then(Value::as_str).unwrap_or("").to_lowercase());
  Ok(values)
}
...
fn capture_screenshot(window: &Window, state: &AppState) -> Result<(), String> {
  #[cfg(target_os = "macos")]
  {
    ...
    let status = Command::new("screencapture")
      ...
      .map_err(|e| e.to_string())?;
    if status.success() {
      window.emit("mt::screenshot-captured", Value::Null).map_err(|e| e.to_string())?;
    }
    Ok(())
  }
  #[cfg(not(target_os = "macos"))]
  {
    emit_notification(window, "Screenshot", "warning", "Interactive screenshots are currently available on macOS builds.")
  }
}

fn handle_format_link_click(window: &Window, state: &AppState, args: Vec<Value>) -> Result<(), String> {
  let payload = args.first().ok_or_else(|| "missing link payload".to_string())?;
  ...
  if pathname.exists() {
    ...
    emit_notification(window, "Link target not found", "error", &format!("{} does not exist.", pathname.to_string_lossy()))?;
  }
  Ok(())
}

fn handle_window_drop(window: &Window, state: &AppState, args: Vec<Value>) -> Result<(), String> {
  let files = args.first().and_then(Value::as_array).cloned().unwrap_or_default();
  for file in files {
...
fn open_markdown_in_window(window: &Window, state: &AppState, path: PathBuf) -> Result<(), String> {
  ...
  window.emit("mt::open-new-tab", vec![document, json!({}), Value::Bool(true)]).map_err(|e| e.to_string())
}
...
fn handle_save_response(window: &Window, state: &AppState, args: Vec<Value>, force_save_as: bool) -> Result<bool, String> {
  ...
  if pathname.is_empty() || force_save_as {
    ...
    window.emit("mt::tab-saved", id).map_err(|e| e.to_string())?;
  }
  Ok(true)
}

fn handle_save_tabs(window: &Window, state: &AppState, args: Vec<Value>) -> Result<Vec<Value>, String> {
  ...
  for file in files {
    ...
    let filename = file.get("filename").and_then(Value::as_str).unwrap_or("Untitled");
    ...
    let default_path = file.get("defaultPath").and_then(Value::as_str).unwrap_or("");
...
fn handle_close_window_confirm(window: &Window, state: &AppState, args: Vec<Value>) -> Result<(), String> {
  let expected = args.first().and_then(Value::as_array).map(|items| items.len()).unwrap_or(0);
  let saved_ids = handle_save_tabs(window, state, args)?;
...
fn handle_file_move_to(window: &Window, args: Vec<Value>) -> Result<(), String> {
  let payload = args.first().ok_or_else(|| "missing move payload".to_string())?;
  let id = payload.get("id").cloned().unwrap_or(Value::Null);
  let pathname = payload.get("pathname").and_then(Value::as_str).ok_or_else(|| "missing pathname".to_string())?;
  let source_path = PathBuf::from(pathname);
  let mut dialog = rfd::FileDialog::new();
  if let Some(parent) = source_path.parent() {
    dialog = dialog.set_directory(parent);
  }
  if let Some(filename) = source_path.file_name().map(|value| value.to_string_lossy().to_string()) {
...
fn handle_file_rename(window: &Window, args: Vec<Value>) -> Result<(), String> {
  let payload = args.first().ok_or_else(|| "missing rename payload".to_string())?;
  let id = payload.get("id").cloned().unwrap_or(Value::Null);
  let pathname = payload.get("pathname").and_then(Value::as_str).ok_or_else(|| "missing pathname".to_string())?;
  let new_pathname = payload.get("newPathname").and_then(Value::as_str).ok_or_else(|| "missing newPathname".to_string())?;
  ...
  if Path::new(new_pathname).exists() {
    emit_notification(
      ...
      "Rename failed",
      "error",
      &format!("{} already exists.", Path::new(new_pathname).file_name().map(|value| value.to_string_lossy()).unwrap_or_default()),
    )?;
    return Ok(());
  }
...
fn recommend_filename(markdown: &str, fallback: &str) -> String {
  markdown
    ...
    .unwrap_or_else(|| sanitize_filename(if fallback.is_empty() { "Untitled" } else { fallback }))
}
...
fn add_recent_document(state: &AppState, path: &str) -> Result<(), String> {
  let mut data = state.data.lock().map_err(|e| e.to_string())?;
  {
    let object = data.as_object_mut().ok_or_else(|| "data store is not an object".to_string())?;
...
fn open_settings_window(app: &AppHandle, category: &str) -> Result<(), String> {
  ...
  if let Some(window) = app.get_webview_window(&label) {
    ...
    if category != "preference" {
      window.emit("settings::change-tab", category).map_err(|e| e.to_string())?;
    }
    return Ok(());
  }
...
fn file_write(path: String, data: Value, append: bool) -> Result<(), String> {
  ...
  if append {
    use std::io::Write;
    let mut file = fs::OpenOptions::new().create(true).append(true).open(path).map_err(map_io)?;
...
fn json_to_bytes(data: Value) -> Result<Vec<u8>, String> {
  match data {
    ...
    Value::Array(items) => items
      .into_iter()
      .map(|item| item.as_u64().and_then(|n| u8::try_from(n).ok()).ok_or_else(|| "byte array contains non-byte value".to_string()))
      .collect(),
  ...
  env::split_paths(&path_var).any(|dir| {
    ...
    #[cfg(windows)]
    {
      for ext in ["exe", "cmd", "bat"] {
        if path_is_executable_file(candidate.with_extension(ext).to_string_lossy().to_string()) {
          return true;
        }
      }
    }
...
#[tauri::command]
fn exec_command(program: String, args: Vec<String>, cwd: String, env: HashMap<String, String>) -> Result<CommandOutput, String> {
...
fn shell_show_item_in_folder(path: String) -> Result<(), String> {
  ...
  {
    Command::new("explorer").arg(format!("/select,{}", path.to_string_lossy())).spawn().map_err(|e| e.to_string())?;
    return Ok(());
  }
...
fn collect_platform_fonts(fonts: &mut BTreeSet<String>) {
  #[cfg(target_os = "macos")]
  {
    if let Ok(output) = Command::new("system_profiler").args(["SPFontsDataType", "-json"]).output() {
...
fn collect_font_files(dir: &Path, fonts: &mut BTreeSet<String>) {
  ...
  for entry in entries.flatten() {
    ...
    let Some(extension) = path.extension().map(|value| value.to_string_lossy().to_lowercase()) else {
      continue;
    };
...
fn window_action(window: Window, action: String, args: Value) -> Result<(), String> {
  ...
  match action.as_str() {
    ...
    "set_fullscreen" => window.set_fullscreen(args.value.unwrap_or(false)).map_err(|e| e.to_string()),
    "set_always_on_top" => window.set_always_on_top(args.value.unwrap_or(false)).map_err(|e| e.to_string()),
