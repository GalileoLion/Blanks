...
export const getLogLevel = () => {
  if (!global.BLANKS_DEBUG_VERBOSE || typeof global.BLANKS_DEBUG_VERBOSE !== 'number' ||
    global.BLANKS_DEBUG_VERBOSE <= 0) {
    return process.env.NODE_ENV === 'development' ? 'debug' : 'info'
  } else if (global.BLANKS_DEBUG_VERBOSE === 1) {
    return 'verbose'
  } else if (global.BLANKS_DEBUG_VERBOSE === 2) {
    return 'debug'
  }
  return 'silly' // >= 3
}
