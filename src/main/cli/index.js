...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to Blanks and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'blanks-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: blanks [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`Blanks: ${global.BLANKS_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'blanks-user-data')
